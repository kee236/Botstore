...
function sendLineNotify() {
    ...
    var formData = {
        message: 'ทำการอัพเดตสต็อคสินค้าเรียบร้อยแล้ว\n' +
            'เริ่มต้นเมื่อ: ' +
            Utilities.formatDate(getDataFromRange('StoreData', 'B2'), 'GMT+7', 'd MMMM Y') +
            ' ' +
            new Date(getDataFromRange('StoreData', 'B3')).toLocaleTimeString('th-TH', 'HH:MM:ss') +
            '\n' +
            'ใช้เวลา ' +
            getDataFromRange('StoreData', 'B4') +
            ' วินาที\n' +
            'บุคคลกระทำการ: ' +
            getDataFromRange('StoreData', 'B1'),
    };
    var options = {
        method: 'post',
...
export { sendLineNotify, replyMessage, MESSAGE_TYPE };
